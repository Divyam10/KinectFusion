...
class TSDF():
    ...
    def __init__(self, vol_dim, intristics, voxel_size=0.02, device="cuda:0"):
        ...
        self.vox_Wcoords = torch.cat([
            (self.vox_coords * voxel_size) + self._vol_origin, torch.ones(len(self.vox_coords), 1, )],
            dim=1).float().to(self.device)

        self.vox_coords = torch.cat([
            self.vox_coords, torch.ones(len(self.vox_coords), 1, )], dim=1).float().to(self.device)

        self.sdf_values = torch.ones(
            (self._vol_dim[0], self._vol_dim[1], self._vol_dim[2])).float().to(self.device)

        self.rgb_values = torch.zeros(
            (self._vol_dim[0], self._vol_dim[1], self._vol_dim[2], 3)).float().to(self.device)

        self.weights = torch.zeros(
            (self._vol_dim[0], self._vol_dim[1], self._vol_dim[2])).float().to(self.device)


        self._vol_origin = torch.as_tensor(self._vol_origin, device=self.device)
        self._voxel_size = torch.as_tensor(self._voxel_size, device=self.device)
    ...
    def integrate(self, depth_image, camera_pose, color_img, sdf_trunc=0.03):
        ...
        with torch.no_grad():
            ...
            color_img = color_img.to(self.device, dtype=torch.float32)
