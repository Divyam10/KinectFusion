...
class SensorWorker(QThread):
    ...
    def __init__(self):
        ...
        self.k = torch.tensor([[fx, 0, px], [0, fy, py], [0, 0, 1]]).to(dtype=torch.float32).to(self.cuda_device)
    ...
    def read_frame(self):
        ...
        depth_frame_data = np.frombuffer(depth_frame.get_buffer_as_uint16(), dtype=np.uint16)
        depth_frame_data = torch.from_numpy(depth_frame_data.astype(np.float32)).to(self.cuda_device).reshape((self.height, self.width))
        #depth_frame_data = torch.flip(depth_frame_data, dims=[1, 0])

        color_frame_data = np.frombuffer(color_frame.get_buffer_as_uint8(), dtype=np.uint8)
        color_frame_data = torch.from_numpy(color_frame_data).to(self.cuda_device).reshape(self.height, self.width, 3)
